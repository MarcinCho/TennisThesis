...
def avgOpponentStats(database, name, date, opprank, match_id):
    ...
    w_df = database.loc[
        ...
        & (
            ...
            & (database["P2_Glicko"] > opprank - 100)
            & (database["P2_Glicko"] < opprank + 100)
        )
    ]
    ...
    l_df = database.loc[
        ...
        & (
            ...
            & (database["P1_Glicko"] > opprank - 100)
            & (database["P1_Glicko"] < opprank + 100)
        )
    ]
    ...
    first_won_serve = (
        (
            (w_df["w_1stWon"].mean() / w_df["w_svpt"].mean())
            + (l_df["l_1stWon"].mean() / l_df["l_svpt"].mean())
        )
        if w_df["w_svpt"].mean() and l_df["l_svpt"].mean() != 0
        else 0
    )

    second_won_serve = (
        (
            (w_df["w_2ndWon"].mean() / w_df["w_svpt"].mean())
            + (l_df["l_2ndWon"].mean() / l_df["l_svpt"].mean())
        )
        if w_df["w_svpt"].mean() and l_df["l_svpt"].mean() != 0
        else 0
    )
    ...
    points_on_return = (
        w_df["l_svpt"].mean() - (w_df["l_1stWon"].mean() + w_df["l_2ndWon"].mean())
    ) + (l_df["w_svpt"].mean() - (l_df["w_1stWon"].mean() + l_df["w_2ndWon"].mean()))

    serve_points_won = (
        (second_won_serve + first_won_serve)
        / (w_df["w_svpt"].mean() + l_df["l_svpt"].mean())
        if w_df["w_svpt"].mean() and l_df["l_svpt"].mean() != 0
        else 0
    )

    ace_probability = (
        w_df["w_ace"].mean() / w_df["w_svpt"].mean()
        + l_df["l_ace"].mean() / l_df["l_svpt"].mean()
        if w_df["w_svpt"].mean() != 0
        else 0
    )

    df_probability = (
        w_df["w_df"].mean() / w_df["w_svpt"].mean()
        + l_df["l_df"].mean() / l_df["l_svpt"].mean()
        if w_df["w_svpt"].mean() != 0
        else 0
    )
